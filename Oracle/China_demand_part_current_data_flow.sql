...
FROM
  ...
  (SELECT T1.IMC_NO,
    ...
    CASE
      ...
      ELSE 'ABO'
    END AS IMC_TYPE_AGG
  FROM DWSAVR02.DWV01021_IMC_MASTER_DIM T1
  WHERE IMC_AFF_KEY_NO =30
  ),
  ORD_HDR AS (
  (SELECT ORD_CURCY,
  ...
  OORDDTA AS
  (SELECT CAST(CASE WHEN ORD_CURCY='360' THEN 'CNY' WHEN ORD_CURCY='050' THEN 'HKD' ELSE 'N/A' END AS VARCHAR2(3 CHAR)) AS "curcy_id",
    CASE
      WHEN COMB_ORD_FLG='N'
      THEN 'false'
      WHEN COMB_ORD_FLG='Y'
    ...
    CASE
      ...
      ELSE
        CASE
          WHEN COALESCE(IMC_TYPES_HIST_ORD.IMC_TYPE_AGG, IMC_TYPES_CURR_ORD.IMC_TYPE_AGG)='ABO'
          THEN VOL_DISTB
          ELSE ORD_DISTB
        END
    END AS "account_id",
  ...
  ON ORD_HDR.ORD_DISTB  =IMC_TYPES_CURR_ORD.IMC_NO
