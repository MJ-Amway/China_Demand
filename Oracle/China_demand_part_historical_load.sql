...
YM_END NUMBER:=0;
BEGIN
 FOR i IN 24236..24240
 /*from 201908 till 201912*/
...
INTO DWSEAI01.CHINA_DEMAND_PART t SELECT   /*+ parallel(s,32)*/ * FROM (WITH 
IMC_TYPES_HIST AS (SELECT T2.IMC_NO,
  ...
  CASE
    ...
    ELSE 'ABO'
  END AS IMC_TYPE_AGG
FROM (SELECT SNAP_MO_YR_KEY_NO, IMC_KEY_NO,  IMC_CNTRY_KEY_NO, BUS_NATR_CD FROM DWSAVR02.DWV00050_IMC_CCYYMM_FACT
WHERE SNAP_MO_YR_KEY_NO=YM AND IMC_CNTRY_KEY_NO IN (36,7)) T1 LEFT JOIN
(SELECT IMC_NO, IMC_KEY_NO FROM DWSAVR02.DWV01021_IMC_MASTER_DIM WHERE IMC_AFF_KEY_NO     =30) T2
USING(IMC_KEY_NO)),
ORD_HDR AS ((SELECT ORD_CURCY, COMB_ORD_FLG, COMB_ORD, ORD, INV_NBR, WHSE, ORD_DT, ORD_CUST, COALESCE(TRIM(CAST(ORD_SRC AS VARCHAR2(240 BYTE))), '*') AS ORD_SRC, VOL_DISTB, ORD_DISTB, SHP_DISTB, ORD_CANC, ORD_SHP, PAY_REQ, ORD_PAY_STAT, ORGNL_ORD, ORD_TYPE, CUST_SRC, ORD_TAX, ORD_VALUE, ORD_HNDLG_PRICE, COALESCE(ORD_TAX/NULLIF(ORD_VALUE-ORD_TAX, 0), 0) AS ORD_TAX_RT FROM DWSATM01.DWT42016_ORD_HDR_OMS
WHERE ORD_CURCY IN ('360', '050')
    AND DISTB_TYPE NOT IN ('W', 'T')
    AND ORD_DT BETWEEN YM*100+1 AND YM*100+31)),
CCPNAUD AS (SELECT COUP_RED_ORD,
        -SUM(COUP_RED_VAL) AS CPN_LC,
        -SUM(COUP_RED_BV)  AS CPN_BV,
        -SUM(COUP_RED_PV)  AS CPN_PV
      FROM DWSATM01.DWT43061_COUP_CHN
      WHERE COUP_ADT_TYP= 'O'
      GROUP BY COUP_RED_ORD
      ),
OORDDTA AS (
SELECT 
CAST(CASE WHEN ORD_CURCY='360' THEN 'CNY' WHEN ORD_CURCY='050' THEN 'HKD' ELSE 'N/A' END AS VARCHAR2(3 CHAR)) AS "curcy_id",
CASE
        WHEN COMB_ORD_FLG='N'
        THEN 'false'
        WHEN COMB_ORD_FLG='Y'
...
/*PARSE_DATETIME('%Y-%m', SUBSTR(CAST(SN_T.SNAP_MO_YR_KEY_NO AS STRING), 1, 4)||'-'||SUBSTR(CAST(SN_T.SNAP_MO_YR_KEY_NO AS STRING), 5, 2)) AS snap_dt,*/
...
FROM ORD_HDR ORD_HDR
    LEFT JOIN
    DWSATM01.dwt42051_ord_sales_chnl ORD_CHNL
...
FROM DWSATM01.DWT42017_ORD_LN_OMS WHERE INTGRT_CNTRY_CD IN ('050', '360')
UNION ALL
SELECT ORD_HDR.ORD AS "ord_id",
9999 AS "ord_ln_id",
CAST('1' AS VARCHAR2(1 CHAR)) AS "ord_ln_disp_cd",
1 as NON_TNA_FLAG,
'PRM'||TRIM(OPRMITM.ITEM_NO) AS "ord_item_cd",
'PRM'||TRIM(OPRMITM.ITEM_NO) AS "ord_item_base_cd",
